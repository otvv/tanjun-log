...
const tanjun = {
  ...
  // (defaults to 'false')
  crash(message, _prefix = '+', _type = 'error', _fake = false, _sufix = '!!!') {
  ...
  trace() {
    ...
    return (tanjun.print(`callback trace: ${(new Error()).stack.split('\t').reverse()}`, 'tanjun', 'info', '-'));
  ...
  callee() {
    ...
    return (tanjun.print(`callback callee: ${(new Error()).stack.split('\n')[1].trim().split('\x20')[1]}\n\t from Object tanjun`, 'tanjun', 'info', '-'));
    // TODO: add current node instance appid to the log info
    // TODO: inject the desired callback into the result so the end user can call this function separately
  },
  ...
  type(any) {
    return (tanjun.print(typeof any, 'tanjun', 'info', '-'));
  },
  ...
  //
  // ARGS: callback = callback that tanjun will try to execute
  ...
  execute(callback, _debug = false, _throwable = false) {
    tanjun.print('attempting to execute callback', 'tanjun', 'normal');
    try {
      callback();
    } catch (e) {
      if (_throwable === true) {
        tanjun.throw(`something went wrong: ${e.message}`, 'tanjun', 'fatal');
      }
    } finally {
      if (_debug === true) {
        const INC_COUNT = 1;
        Array.from(arguments).forEach((args, index) => {
          if (tanjun.type(args) === 'boolean') {
            tanjun.print(`argument ${(index + INC_COUNT)}: boolean ${args}`, 'tanjun', 'info');
          }
          else {
            tanjun.print(`argument ${(index + INC_COUNT)}: ${args}`, 'tanjun', 'info');
          }
        })
        tanjun.print(`arguments being passed: ${arguments.length}`, 'tanjun', 'info');
        tanjun.print(`callback callee: ${(new Error()).stack.split('\n')[1].trim().split('\x20')[1]}\n\t from Object tanjun`, 'tanjun', 'info');
        // get trace stack
        tanjun.trace();
      }
      tanjun.print('stopped callback execution', 'tanjun', 'normal');
    }
  },
}

export default tanjun;
